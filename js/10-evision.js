...
function makeCubicle(W, D, label, bays) {
  ...
  if (bays > 1) {
    ...
    Array.from({ length: bays }, (_, i) => i).forEach(i => {
      ...
      g.add(ch);
      g.add(box(MAT.dark, 0.48, 0.44, 0.46, cx, 0.22, D / 2 - 0.55, false));   // drawers
    });
...
function makeUtilityRoom(W, D, label, kind, omit) {
  ...
  if (kind === 'server') {
    ...
    g.add(box(MAT.wallWhite, 1.0, 0.3, 0.22, 0, H - 0.7, -hd + 0.15, false));   // vent
  } else {
...
function makeHRDesk(W) {
  ...
  mon.position.set(-0.4, 0.78, 0.08);
  g.add(mon);
  g.add(box(MAT.darker, 0.42, 0.02, 0.15, -0.4, 0.8, 0.34, false));
  ...
  const ch = makeChair(MAT.fabric);
  ch.position.set(-0.2, 0, D / 2 + 0.62);
  g.add(ch);
  return g;
}
