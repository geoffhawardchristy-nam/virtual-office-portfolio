...
function registerInteractive(obj, cfg, sc) {
  ...
  let dot = null, rim = null;
  if (cfg.marker === 'arrow') {
    // a big arrow hanging over the thing, so it reads as "come here"
    hMat.color.set(cfg.color || '#F5A524');
    const shaft = new THREE.Mesh(GEO.cyl, hMat);
    shaft.scale.set(0.2, 0.7, 0.2); shaft.position.y = 0.72;
    const head = new THREE.Mesh(GEO.cone, hMat);
    head.scale.set(0.78, 0.6, 0.78); head.rotation.x = Math.PI; head.position.y = 0.2;
    const cap = new THREE.Mesh(GEO.sphere, hMat);
    cap.scale.setScalar(0.22); cap.position.y = 1.07;
    hs.add(shaft); hs.add(head); hs.add(cap);
    rec.arrow = true;
    rec.hotBaseY = h.y || 2;
    const lab = makeMarkerLabel(cfg.label || cfg.title, cfg.color);
    lab.position.set(h.x || 0, (h.y || 2) + 1.55, h.z || 0);
    obj.add(lab);
    rec.label = lab;
  } else {
    dot = new THREE.Mesh(GEO.disc, hMat);
    dot.scale.setScalar(0.14);
    rim = new THREE.Mesh(GEO.torus, hMat);
  ...
  // keep the approach point inside the room
  if (rec.prox && sc.bounds) {
    rec.prox.x = Math.max(-sc.bounds.x, Math.min(sc.bounds.x, rec.prox.x));
    rec.prox.z = Math.max(-sc.bounds.z, Math.min(sc.bounds.z, rec.prox.z));
  }
...
function buildHub(sc) {
  ...
  DOORS.forEach((d, i) => {
    ...
    dr.rec = registerInteractive(dr.group, {
      ...
      label: d.marker || d.company.split(' ')[0],
...
function buildOffice(sc, L) {
  ...
  (L.tables || []).forEach((T, ti) => {
    ...
    g.add(T.pinboard ? makeBenchRun(tw, td, { board: T.board })
      : makeBenchCluster(tw, td, { endStore: T.endStore }));
    sc.root.add(g);
    ...
    (T.seats || []).forEach((seat, i) => {
      ...
      // These offsets were tuned against a 2.3 deep run. Scale them to the
      // actual depth and clamp, or a shallower desk leaves the screen and
      // keyboard hanging over the edge.
      const hdesk = td / 2, dk = hdesk / 1.15;
      // monBack pushes the screen to the back edge instead of sitting it in
      // front of the user — right for a shallow bench against a wall
      const zMon = T.monBack ? -(hdesk - 0.28) : Math.min(0.5 * dk, hdesk - 0.22);
      const zKey = Math.min(1.02 * dk, hdesk - 0.12);
      const zMouse = Math.min(1.05 * dk, hdesk - 0.08);
      const zPot = Math.min(0.95 * dk, hdesk - 0.1);

      const seatG = group(sx, 0, 0);
      const mon = makeMonitor(scr, seat.dual ? 1.12 : 1.02);
      mon.position.set(0, 0.805, side * zMon);
      if (side < 0) mon.rotation.y = Math.PI;
      seatG.add(mon);
      seatG.add(box(MAT.blackSoft, 0.62, 0.022, 0.2, 0, 0.815, side * zKey, false));
      seatG.add(box(MAT.blackSoft, 0.085, 0.03, 0.12, 0.42, 0.82, side * zMouse, false));
      if (i % 2 === 0) seatG.add(cyl(MAT.potWhite, 0.05, 0.1, -0.62, 0.855, side * zPot, true));
  ...
  if (L.whiteboard) {
    ...
    wb.position.set(L.whiteboard.x, L.whiteboard.y || 0, L.whiteboard.z);
  ...
  (L.cubicles || []).forEach(c => {
    ...
    u.rotation.y = c.ry || 0;
    sc.root.add(u);
    // Block the screens and the desk band only — the open side has to stay
    // walkable or nobody can get into the bay. The footprint turns with the
    // cubicle, so every offset goes through toW.
    const cw = c.w || 3.1, cd = c.d || 3.1, cry = c.ry || 0;
    const ca = Math.cos(cry), sa = Math.sin(cry);
    const toW = (lx, lz) => ({ x: c.x + lx * ca + lz * sa, z: c.z - lx * sa + lz * ca });
    const along = Math.abs(ca) > 0.5;
    const put = (lx, lz, hx, hz) => {
      const p = toW(lx, lz);
      blocker(sc, p.x, p.z, along ? hx : hz, along ? hz : hx);
    };
    put(0, -cd / 2 + 0.41, cw / 2, 0.45);              // back screen + desk run
    put(-cw / 2, 0, 0.1, cd / 2);                      // side screens
    put(cw / 2, 0, 0.1, cd / 2);
    for (let i = 1; i < (c.bays || 1); i++)            // divider between bays
      put(-cw / 2 + (cw / c.bays) * i, 0.12, 0.1, (cd - 0.25) / 2);
  });
  ...
  (L.rooms || []).forEach(rm => {
    ...
    const rry = rm.ry === undefined ? Math.PI : rm.ry;
    u.rotation.y = rry;
    sc.root.add(u);
    // a quarter turn swaps the footprint's axes
    const along = Math.abs(Math.cos(rry)) > 0.5;
    blocker(sc, rm.x, rm.z, along ? rm.w / 2 : rm.d / 2, along ? rm.d / 2 : rm.w / 2);
  });
  ...
  (L.fans || []).forEach(([x, z, span, top]) => {
    // same fan as the cabins use, unless a layout asks for another size
    const f = makeCeilingFan(span || FAN_SPAN, top === undefined ? FAN_TOP : top);
    f.position.set(x, 0, z); sc.root.add(f);
  });
